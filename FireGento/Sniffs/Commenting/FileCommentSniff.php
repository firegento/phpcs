...
class FireGento_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected function processCategory($errorPos)
    {
        ...
        if ($category !== null) {
            ...
            if ($content !== '') {
                ...
                } elseif ($content != 'FireGento') { // Category must be the FireGento namespace
                    $error = '@category tag must be "FireGento"';
                    $this->currentFile->addError($error, $errorPos, 'InvalidCategory');
                }
            } else {
    ...
    protected function processPackage($errorPos)
    {
        ...
        // Check if the package matches very likely the module name
        if (strpos($content, 'FireGento_') === false) {
            $error = '@package tag must contain the module name, e.g. "FireGento_ExampleModule"';
            $this->currentFile->addError($error, $errorPos, 'EmptyPackage');
            return;
        }
        ...
        if (empty($authors) === false) {
            foreach ($authors as $author) {
                ...
                if ($content !== '') {
                    ...
                    } elseif ($content != 'FireGento Team <team@firegento.com>') { // Check if correct author is set
                        $error = '@author must be "FireGento Team <team@firegento.com>"';
                        $this->currentFile->addError($error, $errorPos, 'InvalidAuthors');
                    }
                } else {
        ...
        foreach ($copyrights as $copyright) {
            ...
            if ($content !== '') {
                ...
                if (preg_match('/^([0-9]{4})((.{1})([0-9]{4}))? (.+)$/', $content, $matches) !== 0) {
                    ...
                    // Add check if FireGento copyright is found
                    if (strpos($content, 'FireGento Team (http://www.firegento.com)') === false) {
                        $error = '@copyright must contain "FireGento Team (http://www.firegento.com)" after the copyright year';
    ...
    protected function processLicense($errorPos)
    {
        ...
        if ($license !== null) {
            ...
            } elseif ($value != 'http://opensource.org/licenses/gpl-3.0' // Add check if correct license is used
                || $comment != 'GNU General Public License, version 3 (GPLv3)'
            ) {
                $error = '@license must be "http://opensource.org/licenses/gpl-3.0 GNU General Public License, version 3 (GPLv3)"';
                $this->currentFile->addError($error, $errorPos, 'WrongLicense');
            }
        }
    ...
    protected function processVersion($errorPos)
    {
        ...
        if ($version !== null) {
            ...
            // We don't need the following stuff
