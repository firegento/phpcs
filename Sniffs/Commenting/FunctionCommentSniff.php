 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class FireGento_Sniffs_Commenting_FunctionCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected function processReturn($commentStart, $commentEnd)
    {
        ...
        if ($isSpecialMethod === false && $methodName !== $className) {
            ...
            if ($this->commentParser->getReturn() === null) {
                /* We should not report that (void returns make no sense)
    ...
    protected function processParams($commentStart)
    {
        ...
        if (empty($params) === false) {
            ...
            foreach ($params as $param) {
                ...
                // Make sure that there is only one space before the var type if there is no return tag
                // and two spaces when there is a return tag
                if ($this->commentParser->getReturn() === null) {
                    $paramsWhitespaceCheck = ' ';
                } else {
                    $paramsWhitespaceCheck = '  ';
                }
                if ($param->getWhitespaceBeforeType() !== $paramsWhitespaceCheck) {
                    $error = 'Expected %s space(s) before variable type';
                    $data = array(
                        strlen($paramsWhitespaceCheck)
                    );
                    $this->currentFile->addError($error, $errorPos, 'SpacingBeforeParamType', $data);
                }
